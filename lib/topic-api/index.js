...
function onlyForumAdmins(req, res, next) {
  ...
  if (config.multiForum) {
    if (req.forum.isOwner(req.user)) return next();
...
function canVoteAndComment(req, res, next) {
  ...
  if (!config.multiForum || user.staff || !forum.private) {
    return next();
  }
  ...
  if (!forum) {
    return _handleUnauthorizedError(new Error('Forum not found.'), req, res);
  }

  if (forum.isOwner(user) || forum.hasRole(user, 'admin', 'collaborator')) {
    return next();
  } else {
    return _handleUnauthorizedError(new Error('Unauthorized.'), req, res);
  }
